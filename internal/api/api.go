...
func NewGandler(q *pgstore.Queries) http.Handler {
	a := apiHandler{
		q: q,
		upgrader: websocket.Upgrader{
...
func (h apiHandler) validateRoom(rawRoomID string, r *http.Request) Validator {
	roomID, err := uuid.Parse(rawRoomID)

	if err != nil {
		return Validator{OK: false, StatusCode: http.StatusBadRequest, Message: "invalid room id"}
	}

	_, err = h.q.GetRoom(r.Context(), roomID)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return Validator{OK: false, StatusCode: http.StatusBadRequest, Message: "room not found"}
		}

		return Validator{OK: false, StatusCode: http.StatusInternalServerError, Message: "something went wrong"}
	}

	return Validator{OK: true, StatusCode: http.StatusOK, Message: "OK!"}
}
...
func (h apiHandler) handleGetRoomMessages(w http.ResponseWriter, r *http.Request) {
	...
	validator := h.validateRoom(rawRoomID, r)

	if !validator.OK {
		http.Error(w, validator.Message, validator.StatusCode)
		return
	}

	messages, err := h.q.GetRoomMessages(r.Context(), roomID)
	if err != nil {
		slog.Error("failed to get all room message", "error", err)
		http.Error(w, "something went wrong", http.StatusInternalServerError)
		return
	}
